...
def model(x_train,layer_num,hidden_num):
  image_size=28
  num_labels=10
  # layer 1
  ...
  # final layer
  w=tf.Variable(tf.truncated_normal([hidden_num,num_labels]))
  b=tf.Variable(tf.truncated_normal([num_labels]))
  logits=tf.matmul(logits,w)
  logits=tf.add(logits,b)
  return logits

def a2t5(batch_size,learning_rate,layer_num,hidden_num):
  ...
  with graph.as_default():
    ...
    logits=model(x_train,layer_num,hidden_num)
    ...
    test_prediction=model(test_dataset,layer_num,hidden_num)
